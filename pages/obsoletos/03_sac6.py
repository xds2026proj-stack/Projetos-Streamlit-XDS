...
# -----------------------------------------------------------------------------
# 4. APLICAÇÃO DO ESTILO CUSTOMIZADO (CSS MODERNO)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 5. CARREGAMENTO E SALVAMENTO DE DADOS (SEM CACHE DE AUTENTICAÇÃO EXPIRÁVEL)
# -----------------------------------------------------------------------------
def obter_cliente_gspread():
    """Conexão direta renovada a cada operação para evitar erros de sessão expirada."""
...
def carregar_dados():
    ...
    # Tratamento numérico para evitar TypeError ao concatenar str + float
    cols_numericas = [
        "Frete_Receber",
        "Adicional_Receber",
        "Frete_Pagar",
    ...
    for col in cols_numericas:
        df_entregas[col] = (
            pd.to_numeric(df_entregas[col], errors="coerce")
            .fillna(0.0)
            .astype(float)
        )
...
def salvar_dados_na_planilha(df_para_salvar):
    """Executa a gravação criando uma sessão ativa do gspread."""
    gc = obter_cliente_gspread()
    sh = gc.open_by_url(URL_PLANILHA)
...
# -----------------------------------------------------------------------------
# 6. SIDEBAR COM OPÇÃO DE COR & PERFIL
# -----------------------------------------------------------------------------
...
with aba_objs[1]:
    if usuario_atual["perfil"] == "Leitor":
        ...
        st.info(
            "💡 **Aviso:** Alterações em Adicionais informadas pela operação entram como **'Pendente'** para homologação do Financeiro."
        )
...
if tem_acesso_financeiro:
    ...
    with aba_objs[4]:
        ...
        st.caption(
            "Cargas homologadas são agrupadas em Faturas para acompanhamento de cobrança e pagamentos."
        )

        tab_fat_rec, tab_fat_pag = st.tabs(
            ["📥 Contas a Receber (Clientes)", "📤 Contas a Pagar (Terceiros)"]
        )
        ...
        with tab_fat_rec:
            st.markdown("##### 📥 Gestão de Faturas a Receber")
            ...
            df_fat_rec["Frete_Receber"] = pd.to_numeric(
                df_fat_rec["Frete_Receber"], errors="coerce"
            ).fillna(0.0)
            df_fat_rec["Adicional_Receber"] = pd.to_numeric(
                df_fat_rec["Adicional_Receber"], errors="coerce"
            ).fillna(0.0)
            ...
            if st.button("💾 Salvar Status de Faturamento (Receber)", type="primary"):
                try:
        ...
        with tab_fat_pag:
            st.markdown("##### 📤 Gestão de Faturas a Pagar (Terceiros / Agregados)")
            ...
            df_fat_pag["Frete_Pagar"] = pd.to_numeric(
                df_fat_pag["Frete_Pagar"], errors="coerce"
            ).fillna(0.0)
            df_fat_pag["Adicional_Pagar"] = pd.to_numeric(
                df_fat_pag["Adicional_Pagar"], errors="coerce"
            ).fillna(0.0)
            ...
            if st.button("💾 Salvar Status de Pagamentos (Pagar)", type="primary"):
                try:
