...
def aplicar_estilo_e_tema():
    # Carrega Bootstrap CSS e FontAwesome para ícones mais modernos
    st.markdown("""
        <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css">
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    """, unsafe_allow_html=True)

    # Sidebar: Seletor de Tema/Estilo
    st.sidebar.markdown("### 🎨 Aparência do Sistema")
    tema_selecionado = st.sidebar.selectbox(
        "Cor / Estilo do Tema:",
        ["Vermelho Corporativo", "Escuro Moderno (Dark)", "Clássico Bootstrap (Light)", "Slate Tech"],
        index=0
    )

    # Dicionário de paletas CSS dinâmicas
    paletas = {
    ...
    cores = paletas[tema_selecionado]

    # Injeta as variáveis de estilo no Streamlit
    css_customizado = f"""
    <style>
        :root {{
            --bg-primary: {cores['--bg-primary']};
            --bg-card: {cores['--bg-card']};
            --accent-color: {cores['--accent-color']};
            --accent-hover: {cores['--accent-hover']};
            --text-color: {cores['--text-color']};
            --text-muted: {cores['--text-muted']};
            --border-color: {cores['--border-color']};
            --active-tab: {cores['--active-tab']};
        }}
        ...
        /* Cards / Containers Modernos (Efeito de elevação e bordas suaves) */
        ...
        /* Destaques em Vermelho/Cor Ativa para Tabs e Botões */
        ...
        /* Customização de Tabelas e Dataframes */
        .stDataFrame {{
            border: 1px solid var(--border-color) !important;
            border-radius: 8px !important;
            overflow: hidden;
...
COLUNAS_FINANCEIRAS = [
    ...
    "Adicional_Pagar",
    "Custo_Diesel",
    "Status_Adicional_Receber",  # Novo: Validação do Financeiro
    "Status_Adicional_Pagar",  # Novo: Validação do Financeiro
]
...
if "tema_selecionado" not in st.session_state:
    st.session_state.tema_selecionado = "🔵 Azul Corporativo"
...
# -----------------------------------------------------------------------------
# 4. APLICAÇÃO DO ESTILO CUSTOMIZADO (CSS)
# -----------------------------------------------------------------------------
...
if tem_acesso_financeiro:
    ...
    # Operacional pode sugerir Adicionais
    colunas_permitidas = [
        c
        for c in df_raw.columns
        if c
...
# 8. ESTRUTURA DE ABAS
# -----------------------------------------------------------------------------
abas = ["📋 Visão Geral", "⚡ Operação & Edição", "📊 SGI: OTD / Ocorrências"]
if tem_acesso_financeiro:
    abas.append("💳 Validação Financeira por Carga")
    abas.append("💰 Controladoria & DRE")

aba_objs = st.tabs(abas)
...
with aba_objs[0]:
    ...
    st.divider()
    st.dataframe(
        df_filtrado[colunas_permitidas], width="stretch", hide_index=True
    )

# -----------------------------------------------------------------------------
# ABA 2: OPERAÇÃO EM LOTE + EDIÇÃO TABULAR COMPLETA
# -----------------------------------------------------------------------------
with aba_objs[1]:
    if usuario_atual["perfil"] == "Leitor":
        ...
        with st.container(border=True):
            ...
            if c_lote6.button(
                "🚀 Aplicar Alteração em Lote", type="primary", width="stretch"
            ):
        ...
        st.info(
            "💡 **Aviso:** Alterações em Adicionais enviadas pela operação entrarão como **'Pendente'** para aprovação do Financeiro."
        )
        ...
        # Permitir edição de Adicionais mesmo sem perfil financeiro (para sugestão)
        colunas_editaveis = [
            "Motorista",
            "Status",
            "Ocorrência",
        ...
        if st.button("💾 Salvar Edição na Nuvem", type="primary"):
            try:
                with st.spinner("Atualizando planilha na nuvem..."):
                    # Se houve alteração de adicional, volta status para Pendente
                ...
                st.success("✅ Tabela gravada com sucesso!")
                st.cache_data.clear()
                st.rerun()
...
# -----------------------------------------------------------------------------
# ABA 4 & 5: FINANCEIRO COM APROVAÇÃO
# -----------------------------------------------------------------------------
if tem_acesso_financeiro:
    with aba_objs[3]:
        st.markdown(
            "### 💳 Validação Financeira e Homologação de Adicionais por Carga"
        )
        st.caption(
            "Valide ou rejeite os adicionais informados pela equipe operacional antes que eles afetem a DRE."
        )
        ...
        df_cargas_fin = (
            ...
            .agg({
                ...
                "Status": "first",
                "Frete_Receber": "first",
                "Adicional_Receber": "first",
                "Status_Adicional_Receber": "first",
                "Frete_Pagar": "first",
                "Adicional_Pagar": "first",
            ...
            try:
                with st.spinner("Atualizando validações na nuvem..."):
                    for _, row in df_fin_editado.iterrows():
                        ...
                        df_raw.loc[mask, "Adicional_Receber"] = row[
                            "Adicional_Receber"
                        ]
                        ...
                        df_raw.loc[mask, "Adicional_Pagar"] = row[
                            "Adicional_Pagar"
                        ]
    ...
    with aba_objs[4]:
        st.markdown("#### 💰 Resultado Financeiro Consolidado (DRE por Carga)")
        st.caption(
            "⚠️ **Nota do Sistema:** Adicionais só entram no cálculo do DRE quando o status estiver marcado como **Aprovado**."
        )
        ...
        df_calc = (
            ...
            .first()
            .reset_index()
            if "Carga" in df_filtrado.columns
            else df_filtrado
        )

        # Fretes fixos
        rec_frete = pd.to_numeric(
            df_calc["Frete_Receber"], errors="coerce"
        ).sum()
        pag_frete = pd.to_numeric(df_calc["Frete_Pagar"], errors="coerce").sum()
        c_diesel = pd.to_numeric(df_calc["Custo_Diesel"], errors="coerce").sum()

        # Adicionais APROVADOS
        df_adic_rec_aprovados = df_calc[
            df_calc["Status_Adicional_Receber"] == "Aprovado"
        ]
        rec_adic = pd.to_numeric(
            df_adic_rec_aprovados["Adicional_Receber"], errors="coerce"
        ).sum()

        df_adic_pag_aprovados = df_calc[
            df_calc["Status_Adicional_Pagar"] == "Aprovado"
        ]
        pag_adic = pd.to_numeric(
            df_adic_pag_aprovados["Adicional_Pagar"], errors="coerce"
        ).sum()

        # Adicionais PENDENTES (Apenas Informativo)
        df_adic_rec_pend = df_calc[
            df_calc["Status_Adicional_Receber"] == "Pendente"
        ]
        rec_adic_pend = pd.to_numeric(
            df_adic_rec_pend["Adicional_Receber"], errors="coerce"
        ).sum()

        df_adic_pag_pend = df_calc[
            df_calc["Status_Adicional_Pagar"] == "Pendente"
        ]
        pag_adic_pend = pd.to_numeric(
            df_adic_pag_pend["Adicional_Pagar"], errors="coerce"
        ).sum()

        f_total_receber = rec_frete + rec_adic
        f_total_custos = pag_frete + pag_adic + c_diesel
        ...
        st.divider()
        st.markdown("##### ⏳ Adicionais Aguardando Aprovação do Financeiro")
        p1, p2 = st.columns(2)
        p1.warning(f"📥 Adicionais a Receber Pendentes: **R$ {rec_adic_pend:,.2f}**")
        p2.warning(f"📤 Adicionais a Pagar Pendentes: **R$ {pag_adic_pend:,.2f}**")